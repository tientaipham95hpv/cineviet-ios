    ...
    var body: some View {
        NavigationStack {
            ScrollView { VStack(alignment: .leading, spacing: 18) {
                ...
                else if let error { ContentUnavailableView { Label("Không tải được phòng", systemImage: "wifi.exclamationmark") } description: { Text(error) } actions: { Button("Thử lại") { Task { await load() } }.buttonStyle(.borderedProminent) } }
                else if rooms.isEmpty { ContentUnavailableView("Chưa có phòng public", systemImage: "person.2.slash", description: Text("Bạn có thể nhập mã phòng riêng tư ở trên.")) }
